...
static int stats_seq_show(struct seq_file *sf, void *v)
{
	...
	stats.ipi_full_failures =
		(u32)atomic_read(&shared->to_child.full_failures) +
		(u32)atomic_read(&shared->to_parent.full_failures);
	...
	for (i = 0; i < MK_IPI_RING_SIZE; i++) {
		/* Pair with the producer's release publication. */
