...
static int stats_seq_show(struct seq_file *sf, void *v)
{
	...
	struct {
		...
		u32 ipi_to_child_ready;
		u32 ipi_to_parent_ready;
	...
	for (i = 0; i < MK_IPI_RING_SIZE; i++) {
		if (smp_load_acquire(&shared->to_child.entries[i].ready))
			stats.ipi_to_child_ready++;
		if (smp_load_acquire(&shared->to_parent.entries[i].ready))
			stats.ipi_to_parent_ready++;
	}
...
#define MK_STATS_VALUE(name, value) seq_printf(sf, name " %u\n", value)
	MK_STATS_VALUE("ipi.to_child_ready", stats.ipi_to_child_ready);
	MK_STATS_VALUE("ipi.to_parent_ready", stats.ipi_to_parent_ready);
