...
static bool mk_pci_message_from_host(s32 sender_instance_id)
{
	return host_instance && sender_instance_id == host_instance->id;
}
				       ...
				       s32 sender_instance_id, void *ctx)
{
	...
	if (msg_type != MK_MSG_IO || subtype != MK_IO_IRQ_FORWARD ||
	    ...
	    !mk_pci_message_from_host(sender_instance_id))
		return;
