					  ...
					  unsigned int slot)
{
	...
	if (!local_irq || !mk_self || !mk_self->ipi_data ||
...
static int mk_pci_send_irq_request(struct mk_pci_irq_request *request)
{
	...
	request->sender_instance_id = mk_self ? mk_self->id : -1;
	ret = mk_reply_reserve(mk_self->ipi_data, MK_REPLY_PCI_IRQ,
	...
	ret = mk_reply_wait(mk_self->ipi_data, &reply, 1000,
			    &status, NULL);
...
int mk_pci_msi_prepare(struct pci_dev *dev, int nvec, int type)
{
	...
	if (state != MK_PCI_MSI_IDLE && state != MK_PCI_MSI_FAILED)
		return -EBUSY;
	...
	for (;;) {
		...
		for (slot = 0; slot < MK_IRQ_MAILBOX_SLOTS; slot++) {
			struct mk_irq_mailbox_entry *entry =
				&mk_self->ipi_data->irq_mailbox.entries[slot];
			u64 token;
...
int mk_pci_reset_flr(struct pci_dev *dev)
{
	...
	request.request_id = atomic64_inc_return(&mk_pci_request_id);
	request.sender_instance_id = mk_self->id;
	ret = mk_reply_reserve(mk_self->ipi_data, MK_REPLY_PCI_RESET,
			       request.request_id, &reply);
	if (ret)
		return ret;
	request.reply_slot = reply.slot;
	...
	ret = mk_reply_wait(mk_self->ipi_data, &reply,
				...
				bool write, u32 *value)
{
	struct mk_pci_cfg_request request = {
		...
		.lifecycle_epoch = mk_self && mk_self->ipi_data ?
			READ_ONCE(mk_self->ipi_data->spawn_epoch) : 0,
		.sender_instance_id = mk_self ? mk_self->id : -1,
		.domain = domain,
		.bus = bus,
		.devfn = devfn,
		.reg = where,
	...
	ret = mk_reply_reserve(mk_self->ipi_data, MK_REPLY_PCI_CFG,
			       request.request_id, &reply);
	if (ret)
		goto out_error;
	request.reply_slot = reply.slot;
	...
	ret = mk_reply_wait_atomic(mk_self->ipi_data, &reply, 20000,
...
{
	if (!mk_self || !host_instance || !mk_self->pci_devices_valid)
		return 0;
...
static int __init x86_multikernel_pci_init(void)
{
	...
	if (!mk_self->pci_device_count)
		return 0;
	if (!mk_self->pci_devices_valid || !mk_pci_roots_ready)
